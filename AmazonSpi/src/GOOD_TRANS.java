...
public class GOOD_TRANS {

	public static void main(String[] args) {
		
		final int langTranFrom =1;
		String lanF=null;
		if(langTranFrom ==0)
		{lanF="En";}
		...
		try {
		    ...
		    String sqlgetToTransNum = "SELECT COUNT(*) FROM CleaningAgent";
		    ResultSet rsSelect=stmt.executeQuery(sqlgetToTransNum);
		    c.commit();
		    int totalNum = rsSelect.getInt("COUNT(*)");
		    
		    String sqlgetHaveTran = "SELECT COUNT(*) FROM CleaningAgent WHERE name"+lanT+" IS NOT NULL";
			...
			while(leftNum!=0)
			{
				String sqlgetCurrentTranData = "SELECT name"+lanF+",description"+lanF+" FROM CleaningAgent WHERE cleaningAgentID ="+(TranedNum+1);
				...
				for(int i=0;i<2;i++)
				{
			        ...
			        if(i==0)
			        {sqlUpdate = "UPDATE CleaningAgent SET name"+lanT+"='"+resultStr+"' WHERE cleaningAgentID ="+(TranedNum+1);}
			        else
			        {sqlUpdate = "UPDATE CleaningAgent SET description"+lanT+"='"+resultStr+"' WHERE cleaningAgentID ="+(TranedNum+1);}
